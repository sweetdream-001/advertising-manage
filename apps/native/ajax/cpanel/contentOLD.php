...
// Privacy Page
    ...
    $qw = "UPDATE `cl_publications` SET `text` = '".$text."', `category_id` ='".$category."'  WHERE `cl_publications`.`id` = '".$id."'";
    $db->rawQuery($qw);
      ...
      if(isset($_FILES) && !empty($_FILES)){
        ...
        if (isset($image['tmp_name'], $image['size'], $image['name'], $image['type'])) {
    		...
    		if(isset($isExist) && !empty($isExist)){
    			...
    			$db->rawQuery("INSERT INTO `cl_pubmedia` (`pub_id`, `type`, `src`, `json_data`, `time`) VALUES ('".$id."', '".$fileType."', '".$file_upload['filename']."', '".json_encode($jsonData) ."','".time()."')");
    		}
     ...
     function getYoutubeVideoId($url) {
		...
		if(isset($isExist) && !empty($isExist)){
		 ...
		 $resp2 = $db->rawQuery($q2);
			...
			$db->rawQuery("INSERT INTO `cl_pubmedia` (`pub_id`, `type`, `src`, `time`) VALUES ('".$id."', 'yt', '".getYoutubeVideoId($data['yt'])."','".time()."')");
			$resp1 = $db->rawQuery('UPDATE `cl_publications` SET `type` = "yt" WHERE `id`="'.$id.'" ');
